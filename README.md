...
**Desktop**（`packages/desktop`）：Electron + React 18 + TypeScript，使用 electron-vite 构建，Tailwind CSS 做样式，Zustand 管理状态。应用启动时将 server 二进制复制到 `~/.nano-agent/bin/`，以子进程方式运行。
...
应用运行时会在用户主目录下创建 `~/.nano-agent/`，存放配置、二进制文件、日志和会话数据。

```
~/.nano-agent/
├── bin/              # server 和 cloudflared 二进制
├── config/           # 用户配置和 server.json（运行时端口信息）
├── agents/           # Agent 定义文件
├── sessions/         # 会话历史
├── skills/           # 用户自定义 Skill
└── logs/             # 运行日志
