...
export const useSessionStore = create<SessionStore>((set, get) => ({
  ...
  deleteSessionById: async (agentId: string, id: string) => {
    try {
      ...
      if (success) {
        ...
        const { sessions, currentSession, sessionPreviews } = get();
        const newSessions = sessions.filter((s) => s.id !== id);
        const { [id]: _removedPreview, ...remainingPreviews } = sessionPreviews;
        ...
        if (isCurrentDeleted) {
          ...
          set({ sessions: newSessions, sessionPreviews: remainingPreviews });
        }
      }
      return success;
    } catch {
  ...
  /**
   * 更新指定会话的本地预览补丁，覆盖服务端 lastMessage，下次列表刷新后归位。
   * @param sessionId - 会话 ID
   * @param preview - 预览文本
   */
