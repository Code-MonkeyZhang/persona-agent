 ...
 * fire-and-forget 派发整理（多 Agent 同时进行、不互相阻塞）。上次整理时间用
...
/** 安全触发阈值：未处理条目数 ≥ 此值时无视间隔立即整理（避免注入盲区） */
const SAFETY_THRESHOLD = 50;

/** 各 Agent 上次真正派发整理的时间戳（内存记录，重启重置） */
const lastDreamAt = new Map<string, number>();

let timer: ReturnType<typeof setInterval> | null = null;

/**
 * 判断某 Agent 当前是否应触发 Dream（纯函数，便于单测）。
 *
 * - 无未处理料 → 否（no-op，空跑不浪费 LLM 调用）；
 * - 未处理条目数 ≥ {@link SAFETY_THRESHOLD} → 是（安全触发，避免最老的条目
 *   既不注入上下文（注入段最多 50 条）也没被整理，形成失明盲区）；
 * - 否则 → 距上次整理已达 `dreamIntervalMinutes` 才触发。
 *
 * @param agentConfig - Agent 配置（取 dreamIntervalMinutes）
